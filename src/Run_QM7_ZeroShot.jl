...
function main(llm)
    ...
    system_prompt = """You are an expert quantum chemist and know the QM7 dataset very well. You are given a SMILES string of a molecule. Your task is to predict the atomization energy of that molecule in kcal/mol. Provide only the numerical value as output, without any additional text."""

    get_prediction_prompt(smiles) = "What is the atomization energy in the QM7 dataset in kcal/mol of the molecule with the following SMILES string: $smiles? Provide only the numerical value as output, without any additional text."
