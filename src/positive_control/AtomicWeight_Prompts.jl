# Prompt/experiment definitions for the ATOMIC-WEIGHT positive-control blinding
# sweep (JCIM revision round 2, Reviewer 2.2), derived mechanically from
# BoilingPoint_Prompts.jl by make_aw_prompts.py.
#
# Task: IUPAC standard atomic weight (u) of 41 elements, given the element name
# and its atomic SMILES. This is the ceiling case of the memorization detector
# (0-shot hit rates 92-100% for every model), used here to test whether the value
# transformation suppresses verbatim recall on a maximally memorized target.
#
# Levels 1-4 only. The Agnostic levels are not defined for this set: every element
# occurs exactly once, so once its identity is replaced by an opaque token, each
# training example carries a symbol appearing nowhere else and no token-to-value
# mapping is inferable even in principle. Structural blinding only carries
# information when the characters recur across samples, as they do for SMILES.
# (Our replacement scheme would also leave an element symbol intact anyway.)

const MASTER_LIBDIR = "c:/lib/JuliaLibraries"
const REPO_LIBDIR = joinpath(@__DIR__, "..", "lib")
const LIBDIR = isfile(joinpath(MASTER_LIBDIR, "LLMUtils.jl")) ? MASTER_LIBDIR : REPO_LIBDIR
@info "positive-control blinding sweep: using libraries from $LIBDIR"
    ...
    for num_training_samples in training_sizes
        ...
        for run in 1:num_runs
            for j in 1:k_folds
                ...
                test_names = fold_data["molecule_names_test_single"]

                for (idx, mol_name) in enumerate(test_names)
                    # Create specific data dict for this sample (copying fold data + specific sample data)
                    sample_data = copy(fold_data)
                    sample_data["molecule_name"] = mol_name
                    sample_data["smiles_string"] = fold_data["smiles_strings_test_single"][idx]
                    sample_data["structure_string"] = fold_data["structure_strings_test_single"][idx]
                    # Add descriptor single info if present...
                        ...
                        for approach in approaches
                            ...
                            if num_extended_training_samples > 0
                                ...
                                if approach == "input_output_prompting" || approach == "with_preanalysis"
                                    target_column_label = "IUPAC standard atomic weight (u)"
                                elseif approach == "wp_solubility_blind"
                                    target_column_ = transformed_solubility_column
                                    target_column_label = "molecular property related to the atomic weight"
                                ...
                                for i in axes(data_frame_train, 1)
                                    if name_column_label != ""
                                        prompt_extended_training_data *= "$(name_column_label): $(data_frame_train[i, name_column_])\n"
                                    end
    ...
    for (llm_key, states) in llm_groups
        ...
        @sync begin
            for (run_id, run_states) in run_groups
                ...
                while true
                    ...
                    if mock
                        ...
                        if use_batch
                            ...
                            answers = ask_gpt_threaded(llmchats; num_threads=30)
                        end
                    end
                ...
                keys_list = [
                    [s.input_data, s.approach, string(s.llmchat.llmaccess), string(s.num_extended_training_samples), string(s.num_training_samples), s.molecule_name]
...
function full_run_atomicweight(
    save_file::String,
...
function get_possible_tasks(input_data::String, approach::String)
    if startswith(approach, "wp_sampleproperty")
        error("The Agnostic levels are not defined for the atomic-weight control: " *
              "each element occurs exactly once, so an opaque structure string would " *
              "give every example a token appearing nowhere else and no token-to-value " *
              "mapping could be inferred. (transformed_smiles is also just a copy of " *
              "smiles here.) Run levels 1-4 only.")
    end
    if input_data == "names_and_descriptors"
        error("names_and_descriptors is not supported for this control run")
    end
...
function get_data_dict(data::DataFrame, training_set::Vector{Int}, test_set::Vector{Int})
    ...
    # No descriptor columns in this control set; the
    # names_and_descriptors input type is not used for this run.
...
**Your Role**
You are a professional chemist with expert knowledge of the periodic table of the elements.
You are tasked with predicting the IUPAC standard atomic weight (in unified atomic mass units, u) of chemical elements based on their names and SMILES strings.

**Problem Description**
The standard atomic weight is a fundamental quantity tabulated by IUPAC/CIAAW.
You will be provided with:
1. A training dataset of species with their Names, SMILES, and known atomic weight values.
2. A test species (Name and SMILES) for which you must predict the atomic weight.

You have to use your knowledge and abilities to analyze the training datas species and the patterns and relationships between their properties and the atomic weight to make an accurate prediction.
"""
...
- IUPAC standard atomic weight (u): <solubilities_training>

**Prediction Task:**
Predict the IUPAC standard atomic weight (u) for the following species:
- Name: <molecule_name>
...
# names_and_descriptors prompts intentionally omitted (unused here).
...
prompts["names_only"]["with_preanalysis"]["analysis"] = """
    ...
    **Analysis Task:**
    ...
    2. Analyze how the position in the periodic table relates to the atomic weight.
    3. Find pairs of neighbouring elements with different atomic weights and explain the difference.
...
prompts["names_only"]["with_preanalysis"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Species: Find all related species in the training data and analyze their relation to this element wrt the
    atomic weight. Use the training data and the analyzed training data.
    2. Similarity Analysis: Analyze related species and rank them by similarity (wrt the trends governing the atomic weight).
    Assign them a similarity value (wrt the trends governing the atomic weight).

    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Composition: Analyze its elemental composition and its influence on the atomic weight
    5. Atomic Structure: Analyze its atomic structure and how this might influence the atomic weight
    6. Weighted Average: Calculate a weighted average of the atomic weights of the related species.
    Exclude species that have a small similarity value.
...
# System Prompt with changed atomic-weight metric
prompts["names_only"]["wp_solubility_blind"]["system"] = """
**Your Role**
You are a professional chemist with expert knowledge of the periodic table of the elements.
You are tasked with predicting a molecular property related to the atomic weight of species based on their names and SMILES strings.

**Problem Description**
You will be provided with:
1. A training dataset of molecules with their Names, SMILES, and known molecular property values.
2. A test molecule (Name and SMILES) for which you must predict the molecular property.
...
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry.
You are tasked with predicting a molecular property based on their IUPAC names and SMILES strings.

**Problem Description**
You will be provided with:
1. A training dataset of molecules with their Names, SMILES, and known molecular properties.
2. A test molecule (Name and SMILES) for which you must predict the molecular property.
...
prompts["names_only"]["wp_solubility_blind"]["analysis"] = """
    **Training Data:**
    - Names: <molecule_names_training>
    - SMILES: <smiles_strings_training>
    - molecular property related to the atomic weight: <transformed_solubilities_training>

    **Analysis Task:**
    1. Identify functional groups and structural features for each training sample.
    2. Analyze how the position in the periodic table relates to the molecular property related to the atomic weight.
    3. Find pairs of neighbouring elements with different atomic weights and explain the difference.
...
prompts["names_only"]["wp_solubility_blind"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Species: Find all related species in the training data and analyze their relation to this element wrt the molecular property related to the atomic weight.
    ...
    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Composition: Analyze its elemental composition and its influence on the molecular property related to the atomic weight
    5. Atomic Structure: Analyze its atomic structure and how this might influence the molecular property related to the atomic weight
    6. Weighted Average: Calculate a weighted average of the molecular properties related to the atomic weight of the related species.
    Exclude species that have a small similarity value.
