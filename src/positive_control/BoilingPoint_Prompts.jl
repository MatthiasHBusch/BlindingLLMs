# Prompt/experiment definitions for the POSITIVE-CONTROL blinding sweep
# (JCIM revision round 2, Reviewer 2.2).
#
# Derived mechanically from ../Delaney_Prompts.jl by make_bp_prompts.py: only the
# dataset columns and the property wording differ. The six approach names, the
# fold/state machinery and the JSON output format are byte-identical to the
# benchmark runs, so the blinding levels here mean exactly what they mean in
# Figures 3 and 4.
#
# Task: normal boiling point (degrees Celsius, English-Wikipedia infobox value)
# of 45 common compounds -- a set the models demonstrably recall verbatim
# (see positive_control_results.csv). Blinding it answers R2's question: which
# blinding level is sufficient to break verbatim recall?
                        ...
                        for approach in approaches
                            ...
                            if num_extended_training_samples > 0
                                ...
                                if approach == "input_output_prompting" || approach == "with_preanalysis"
                                    target_column_label = "normal boiling point (degrees Celsius)"
                                elseif approach == "wp_solubility_blind"
                                    target_column_ = transformed_solubility_column
                                    target_column_label = "molecular property related to the boiling point"
...
function full_run_boilingpoint(
    save_file::String,
...
function get_possible_tasks(input_data::String, approach::String)
    if input_data == "names_and_descriptors"
        error("names_and_descriptors is not supported for the boiling-point control run")
    end
...
function get_data_dict(data::DataFrame, training_set::Vector{Int}, test_set::Vector{Int})
    ...
    # No descriptor columns in the boiling-point control set; the
...
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry and phase-transition properties.
You are tasked with predicting the normal boiling point (in degrees Celsius, at 1 atm) of compounds based on their names and SMILES strings.

**Problem Description**
The normal boiling point is a key physical property in process chemistry and separation science.
You will be provided with:
1. A training dataset of molecules with their Names, SMILES, and known boiling point values.
2. A test molecule (Name and SMILES) for which you must predict the boiling point.

You have to use your knowledge and abilities to analyze the training datas molecules and the patterns and relationships between molecular properties and the boiling point to make an accurate prediction.
"""
...
- Normal boiling point (degrees Celsius): <solubilities_training>

**Prediction Task:**
Predict the normal boiling point (degrees Celsius) for the following molecule:
- Name: <molecule_name>
...
prompts["names_only"]["with_preanalysis"]["analysis"] = """
    ...
    **Analysis Task:**
    ...
    2. Analyze how these features influence the boiling point (e.g., hydrogen bonding, polarity, molecular size).
    3. Find pairs of similar molecules with different boiling points and explain the difference.
...
prompts["names_only"]["with_preanalysis"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Molecules: Find all similar molecules in the training data and analyze their relation to this compound wrt the boiling
    behaviour. Use the training data and the analyzed training data.
    2. Similarity Analysis: Analyze similar molecules and rank them by similarity (wrt the mechanisms governing the boiling point).
    Assign them a similarity value (wrt the mechanisms governing the boiling point).

    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Functional Groups: Analyze its functional groups and their influence on the boiling point
    5. Atomic Structure: Analyze its atomic structure and how this might influence the boiling point
    6. Weighted Average: Calculate a weighted average of the boiling points of the similar molecules.
...
# System Prompt with changed boiling-point metric
prompts["names_only"]["wp_solubility_blind"]["system"] = """
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry and phase-transition properties.
You are tasked with predicting a molecular property related to the boiling point of compounds based on their names and SMILES strings.
...
prompts["names_only"]["wp_solubility_blind"]["analysis"] = """
    **Training Data:**
    - Names: <molecule_names_training>
    - SMILES: <smiles_strings_training>
    - molecular property related to the boiling point: <transformed_solubilities_training>

    **Analysis Task:**
    1. Identify functional groups and structural features for each training sample.
    2. Analyze how these features influence the molecular property related to the boiling point (e.g., hydrogen bonding, polarity, molecular size).
    3. Find pairs of similar molecules with different boiling points and explain the difference.
...
prompts["names_only"]["wp_solubility_blind"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Molecules: Find all similar molecules in the training data and analyze their relation to this compound wrt the molecular property related to the boiling point.
    ...
    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Functional Groups: Analyze its functional groups and their influence on the molecular property related to the boiling point
    5. Atomic Structure: Analyze its atomic structure and how this might influence the molecular property related to the boiling point
    6. Weighted Average: Calculate a weighted average of the molecular properties related to the boiling point of the similar molecules.
    Exclude molecules that have a small similarity value.
