        ...
        for run in 1:num_runs
            for j in 1:k_folds
                ...
                for (idx, smiles_string) in enumerate(test_smiles)
                    ...
                    # Lipophilicity doesn't have descriptors
                        ...
                        for approach in approaches
                            ...
                            if num_extended_training_samples > 0
                                ...
                                if approach == "input_output_prompting" || approach == "with_preanalysis"
                                    target_column_label = "lipophilicity"
                                elseif approach == "wp_solubility_blind"
                                    target_column_ = transformed_solubility_column
                                    target_column_label = "molecular property related to lipophilicity"
...
function full_run_lipophilicity(
    save_file::String,
...
function get_data_dict(data::DataFrame, training_set::Vector{Int}, test_set::Vector{Int})
    ...
    # Lipophilicity doesn't have descriptors - commenting out
    ...
    # Lipophilicity doesn't have descriptors - commenting out all descriptor processing
...
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry and lipophilicity prediction.
You are tasked with predicting the lipophilicity (logD octanol/water partition coefficient at pH 7.4) of organic molecules in water/octanol based on their SMILES strings.

**Problem Description**
You will be provided with:
1. A training dataset of molecules with their SMILES and known logD values.
2. A test molecule (SMILES) for which you must predict the logD.

You have to use your knowledge and abilities to analyze the training datas molecules and the patterns and relationships between molecular properties and lipophilicity to make an accurate prediction.
"""

prompts["names_only"]["input_output_prompting"]["prediction"] = """
**Training Data:**
- SMILES: <smiles_strings_training>
- LogD lipophilicity: <solubilities_training>

**Prediction Task:**
Predict the logD lipophilicity for the following molecule:
...
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry and lipophilicity prediction.
You are tasked with predicting the lipophilicity (logD octanol/water partition coefficient at pH 7.4) of organic molecules in water/octanol based on their SMILES strings and molecular descriptors.

**Problem Description**
You will be provided with:
1. A training dataset with SMILES, Descriptors (MW, Rings, Rotatable Bonds, H-Donors, TPSA), and logD values.
2. A test molecule with its descriptors.
...
**Prediction Task:**
Predict the logD lipophilicity for:
- SMILES: <smiles_string>
...
prompts["names_only"]["with_preanalysis"]["analysis"] = """
    ...
    **Analysis Task:**
    ...
    2. Analyze how these features influence the lipophilicity (e.g., hydrophilic vs hydrophobic groups).
    3. Find pairs of similar molecules with different lipophilicities and explain the difference.
...
prompts["names_only"]["with_preanalysis"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Molecules: Find all similar molecules in the training data and analyze their relation to this compound wrt the lipophilicity 
    process. Use the training data and the analyzed training data.
    2. Similarity Analysis: Analyze similar molecules and rank them by similarity (wrt the mechanisms in the lipophilicity). 
    Assign them a similarity value (wrt the mechanisms in the lipophilicity).
    
    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Functional Groups: Analyze its functional groups and their influence on the lipophilicity
    5. Atomic Structure: Analyze its atomic structure and how this might influence the lipophilicity
    6. Weighted Average: Calculate a weighted average of the lipophilicities of the similar molecules. 
...
**Your Role**
You are a professional chemist with expert knowledge in physical chemistry and lipophilicity prediction.
You are tasked with predicting a molecular property related to lipophilicity of organic molecules in water/octanol based on their SMILES strings.

**Problem Description**
You will be provided with:
1. A training dataset of molecules with their SMILES and known values of the molecular property related to lipophilicity.
...
You are tasked with predicting a molecular property based on SMILES strings.
...
prompts["names_only"]["wp_solubility_blind"]["analysis"] = """
    **Training Data:**
    - SMILES: <smiles_strings_training>
    - molecular property related to lipophilicity: <transformed_solubilities_training>

    **Analysis Task:**
    1. Identify functional groups and structural features for each training sample.
    2. Analyze how these features influence the molecular property related to lipophilicity (e.g., hydrophilic vs hydrophobic groups).
    3. Find pairs of similar molecules with different lipophilicities and explain the difference.
...
prompts["names_only"]["wp_solubility_blind"]["prediction"] = """
    ...
    **Similar Molecules Relations**
    1. Similar Molecules: Find all similar molecules in the training data and analyze their relation to this compound wrt the molecular property related to lipophilicity. 
    ...
    **Molecule Analysis**
    3. Pattern Analysis: Analyze if found patterns apply to the molecule
    4. Functional Groups: Analyze its functional groups and their influence on the molecular property related to lipophilicity
    5. Atomic Structure: Analyze its atomic structure and how this might influence the molecular property related to lipophilicity
    6. Weighted Average: Calculate a weighted average of the molecular properties related to lipophilicity of the similar molecules. 
    Exclude molecules that have a small similarity value.
